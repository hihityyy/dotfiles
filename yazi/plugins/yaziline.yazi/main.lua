...
local function setup(_, options)
	...
	local config = {
		...
		default_files_color = options.default_files_color
			...
			or "darkgray",
		selected_files_color = options.selected_files_color or th.mgr.count_selected:bg() or "white",
		yanked_files_color = options.yanked_files_color or th.mgr.count_copied:bg() or "green",
	...
	function Status:mode()
		...
		return ui.Line({
			...
			ui.Span(" " .. mode .. " "):fg(config.mask_color):bg(config.color or style.main:bg()):bold(),
		})
	end
	...
	function Status:length()
		...
		local style = self:style()
		return ui.Line({
			ui.Span(current_separator_style.separator_close .. " "):fg(config.color):bg(config.secondary_color),
			ui.Span(ya.readable_size(len) .. " "):fg(config.default_files_color):bg(config.secondary_color):bold(),
		})
	end
	...
	function Status:name()
		...
		if not h then
			return ui.Line({
				...
				ui.Span("Empty dir"):fg(config.default_files_color):bold(),
			})
		end
		...
		return ui.Line({
			ui.Span(current_separator_style.separator_close .. " ")
				:fg(config.secondary_color or th.which.separator_style:fg()),
			ui.Span(truncated_name):fg(config.default_files_color):bold(),
		})
	end
	...
	function Status:files()
		...
		local selected_fg = files_selected > 0 and config.selected_files_color or config.default_files_color
		local yanked_fg = files_yanked > 0 and (files_cut and config.cut_files_color or config.yanked_files_color)
	...
	function Status:percent()
		...
		local style = self:style()
		return ui.Line({
			ui.Span(" " .. current_separator_style.separator_open):fg(config.secondary_color),
			ui.Span(percent):fg("#ffffff"):bg(config.secondary_color):bold(),
			ui.Span(current_separator_style.separator_open):fg(config.color):bg(config.secondary_color),
		})
	end
	...
	Status:children_remove(Status.perm, Status.RIGHT)
end
